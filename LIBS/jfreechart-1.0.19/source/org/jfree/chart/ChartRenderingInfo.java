 ...
 * <code>Graphics2D</code>. It is assumed that client code will sometimes render
 * the same chart in more than one view, so the {@link JFreeChart} instance does
 * not retain any information about its rendered dimensions. This information
 * can be useful sometimes, so you have the option to collect the information at
 * each call to
...
public class ChartRenderingInfo implements Cloneable, Serializable {
     ...
     * intended that you can set this to
     * <code>null</code> to prevent the information being collected.
     */
