 ...
 * there will be spaces between bars for a given x. The example shown here is
 * generated by the
...
public class ClusteredXYBarRenderer extends XYBarRenderer
    ...
    /**
     * Iterates over the items in an {@link IntervalXYDataset} to find the range
     * of x-values including the interval OFFSET so that it centers the interval
     * around the start value.
     *
    ...
    /**
     * Draws the visual representation of a single data item. This method is
     * mostly copied from the superclass, the change is that in the calculated
     * space for a singe bar we draw bars for each series next to each other.
     * The width of each bar is the available width divided by the number of
     * series. Bars for each series are drawn in order left to right.
     *
