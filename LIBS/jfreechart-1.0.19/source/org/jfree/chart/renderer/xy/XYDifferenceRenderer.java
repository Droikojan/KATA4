...
public class XYDifferenceRenderer extends AbstractXYItemRenderer
     ...
     * rounded to integers. When set to true, this can avoid the vertical
     * striping that anti-aliasing can generate. However, the rounding may not
     * be appropriate for output in high resolution formats (for example, vector
     * graphics formats such as SVG and PDF).
     *
     * @since 1.0.4
     */
    ...
    /**
     * Sets the flag that controls whether or not the x-coordinates (in Java2D
     * space) are rounded to integer values, and sends a
     * {@link RendererChangeEvent} to all registered listeners.
     *
     ...
     * to subsequent calls to the drawItem() method. This method will be called
     * before the first item is rendered, giving the renderer an opportunity to
     * initialise any state information it wants to maintain. The renderer can
     * do nothing if it chooses.
     *
    ...
    /**
     * Returns
     * <code>2</code>, the number of passes required by the renderer. The
     ...
     * @param x_plot the plot (can be used to obtain standard color information
     * etc).
    ...
    /**
     * Determines if a dataset is degenerate. A degenerate dataset is a dataset
     * where either series has less than two (2) points.
     *
     * @param x_dataset the dataset.
    ...
    /**
     * Determines if the two (2) series are disjoint. Disjoint series do not
     * overlap in the domain space.
     *
     * @param x_dataset the dataset.
     *
     * @return true if the dataset is degenerate.
     ...
     * @param x_domainAxis the domain (horizontal) axis.
     * @param x_rangeAxis the range (vertical) axis.
     * @param x_positive indicates if the polygon is positive (true) or negative
     * (false).
     * @param x_xValues a linked list of the x values (expects values to be of
     * type Double).
     * @param x_yValues a linked list of the y values (expects values to be of
     * type Double).
     */
    private void createPolygon(Graphics2D x_graphics,
        ...
        if (PlotOrientation.VERTICAL == l_orientation) {
            double l_x = x_domainAxis.valueToJava2D(((Double) l_xValues[0]).doubleValue(), x_dataArea,
                    l_domainAxisLocation);
            if (this.roundXCoordinates) {
                l_x = Math.rint(l_x);
            }

            double l_y = x_rangeAxis.valueToJava2D(((Double) l_yValues[0]).doubleValue(), x_dataArea,
                    l_rangeAxisLocation);

            l_path.moveTo((float) l_x, (float) l_y);
            for (int i = 1; i < l_xValues.length; i++) {
                l_x = x_domainAxis.valueToJava2D(((Double) l_xValues[i]).doubleValue(), x_dataArea,
                        l_domainAxisLocation);
                if (this.roundXCoordinates) {
                    l_x = Math.rint(l_x);
                }

                l_y = x_rangeAxis.valueToJava2D(((Double) l_yValues[i]).doubleValue(), x_dataArea,
                        l_rangeAxisLocation);
